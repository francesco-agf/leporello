...
| file | cos'è |
|---|---|
| `sorgente/leporello.html` | il **sorgente di lavoro**. Non ha doctype né `<head>`: quando lo si pubblica come artifact su claude.ai è la cornice a metterli. È qui che si modifica. |
...
**La griglia si gira.** Il campo è 19 × 13 sul computer e 13 × 19 sul telefono: `COLS` e `ROWS`
sono variabili, non costanti, e cambiano dentro `resize()` — ma **solo fuori partita**, mai
mentre si gioca. Chi scrive una prova che dipende dalle dimensioni del campo deve leggerle
allo stato, non darle per scontate.

**Il gioco avanza a passi, non a millisecondi.** Le prove non aspettano il tempo reale:
chiamano `__leporello.passo(n)` e leggono `__leporello.stato()`.
