/* Collaudo della classifica di Leporello — stessa forma di quella di Baseline
...
(async () => {
  ...
  const t2 = await page.evaluate(async () => {
    ...
    const fase = window.__leporello.stato().fase;
    window.__leporello.fine();
  ...
  ok('3. il punteggio parte', t3.partito === 'play' && t3.nome === 'Francesco' &&
      /su .* piegator/.test(t3.msg), { msg: t3.msg });
  ok('4. il corpo dell\'invio', corpo.gioco === 'leporello' && corpo.name === 'Francesco' &&
  ...
  const t5 = await page.evaluate(async () => {
    const L = window.__leporello;
    document.getElementById('startBtn').click();       // «Un altro foglio»
    await new Promise(r => setTimeout(r, 200));
    L.fine();
    await new Promise(r => setTimeout(r, 700));
